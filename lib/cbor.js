...
var CBOR = (function () {
	...
	// Support for BigInts
	// @ts-ignore
	API.addEncoder(0, value => {
		if (typeof value === "bigint") {
			return value + ""
		}
	})
	// @ts-ignore
	API.addDecoder(0, value => BigInt(value))

	// Support for RegExp
	API.addEncoder(1, value => {
		if (value instanceof RegExp) {
			return value.source + "___flags___" + value.flags
		}
	}).addDecoder(1, value => RegExp.call(null, ...value.split("___flags___")))

	// Support for Date
	API.addEncoder(2, value => {
		if (value instanceof Date) {
			return value.valueOf()
		}
	})
	API.addDecoder(2, value => new Date(value))

	// Support for Map
	API.addEncoder(3, value => {
		if (value instanceof Map) {
			return [ ...value.entries() ]
		}
	})
	API.addDecoder(3, value => new Map(value))

	// Support for Set
	API.addEncoder(4, value => {
		if (value instanceof Set) {
			return [ ...value.values() ]
		}
	})
	API.addDecoder(4, value => new Set(value))
