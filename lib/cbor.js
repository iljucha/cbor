...
if (BigInt && typeof BigInt === "function") {
	CBOR.addEncoder(0, value => {
		if (typeof value === "bigint") {
			return value + ""
		}
		// @ts-ignore
	}).addDecoder(0, value => BigInt(value))
}

// Support for RegExp
CBOR.addEncoder(1, value => {
...
// Support for Date
CBOR.addEncoder(2, value => {
	if (value instanceof Date) {
		return value.valueOf()
	}
}).addDecoder(2, value => new Date(value))

// Support for Map
CBOR.addEncoder(3, value => {
	if (value instanceof Map) {
		return [ ...value.entries() ]
	}
}).addDecoder(3, value => new Map(value))

// Support for Set
CBOR.addEncoder(4, value => {
	if (value instanceof Set) {
		return [ ...value.values() ]
	}
}).addDecoder(4, value => new Set(value))
